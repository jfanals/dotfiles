...
myWorkspaces =
  [
    "7:Notes",    "8:SyA", "9:TechB",
    "4:Bank",  "5:Nat", "6:FB.YT",
    "1:TermL",  "2:TermR", "3:Mail",
    "0:Files",  "E1:Dbg", "E2:Pix"
  ]

startupWorkspace = "5:Nat"  -- which workspace do you want to be on after launch?
...
-- layouts.
myLayouts =
  onWorkspace "7:Notes" chatLayout
  $ onWorkspace "E2:Pix" gimpLayout
  $ defaultLayouts
...
myKeyBindings =
  [
    ...
    , ((myModMask .|. altMask, xK_space), spawn "kupfer")
    , ((myModMask .|. shiftMask, xK_z), spawn "dbus-launch pcmanfm")
    , ((myModMask .|. shiftMask, xK_f), spawn "firefox")
    {-, ((myModMask .|. shiftMask, xK_d), spawn "dropbox")-}
    {-, ((myModMask .|. shiftMask, xK_b), spawn "btsync-gui")-}
    {-, ((myModMask .|. shiftMask, xK_i), spawn "inkscape")-}
    {-, ((myModMask .|. shiftMask, xK_k), spawn "kdenlive")-}
    {-, ((myModMask .|. shiftMask, xK_r), spawn "redshift -l 41.423824:2.148811")-}
    {-, ((myModMask .|. shiftMask, xK_l), spawn "redshift -x")-}
    , ((myModMask, xK_Print), spawn "gnome-screenshot -i")
    {-, ((myModMask .|. altMask, xK_j), spawn "xautolock -disable & notify-send -t 50 -i gtk-dialog-info 'xautolock -disable'")-}
    {-, ((myModMask .|. altMask, xK_k), spawn "xautolock -enable & notify-send -t 50 -i gtk-dialog-info 'xautolock -enable'")-}
    , ((myModMask .|. altMask, xK_j), spawn "xautolock -disable & echo  'xautolock -disable' |  wall")
    , ((myModMask .|. altMask, xK_k), spawn "xautolock -enable & echo 'xautolock -enable' |  wall")
    {-, ((myModMask, xK_j), spawn "amixer -q set Master 10%-")-}
    {-, ((myModMask, xK_k), spawn "amixer -q set Master 10%+")-}
    , ((myModMask, xK_Pause), spawn "sudo dbus-send --system --print-reply --dest=org.freedesktop.login1 /org/freedesktop/login1 'org.freedesktop.login1.Manager.Suspend' boolean:true")
...
myManagementHooks = [
  resource =? "kupfer" --> doIgnore
  , resource =? "stalonetray" --> doIgnore
  , className =? "rdesktop" --> doFloat
  , isFullscreen --> doFullFloat
  , (className =? "Komodo IDE") --> doF (W.shift "5:Nat")
  , (className =? "Komodo IDE" <&&> resource =? "Komodo_find2") --> doFloat
  , (className =? "Komodo IDE" <&&> resource =? "Komodo_gotofile") --> doFloat
  , (className =? "Komodo IDE" <&&> resource =? "Toplevel") --> doFloat
  , (className =? "Empathy") --> doF (W.shift "7:Notes")
  , (className =? "Pidgin") --> doF (W.shift "7:Notes")
  , (className =? "Gimp-2.8") --> doF (W.shift "E2:Pix")
  ]
